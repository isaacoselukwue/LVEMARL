...
from policies.mappo_ensemble import MAPPOEnsemble
from policies.mappo import MAPPOBuffer
...
class MAPPOEnsembleAgent(agents.BaseAgent):
    """
    Agent orchestrator for MAPPO Ensemble, with optional LLM assistance and comprehensive logging.
    """
    def __init__(self, config_path=None, config=None, use_simple_opponents=False, **kwargs):
        ...
        super(MAPPOEnsembleAgent, self).__init__(**kwargs)
        ...
        mappo_config = self.config['policy']['mappo_ensemble']
        ...
        self.policy = MAPPOEnsemble(
            ...
            ensemble_size=mappo_config['ensemble_size'], hidden_sizes=tuple(mappo_config['hidden_sizes']),
            lr=mappo_config['lr'], gamma=mappo_config['gamma'], clip_ratio=mappo_config['clip_ratio'],
        ...
        if self.use_simple_opponents:
            ...
            self.agent_list = [self] + [MAPPOEnsembleAgent(config={}, _shared_policy=self.policy, _featurizer=self.featurizer) for _ in range(3)]
        ...
        self.logger = CsvLogger(log_dir=self.config['trainer']['log_dir'], config=self.config, llm_assisted=self.is_llm_assisted)
    ...
    def train(self, is_tuning=False):
        if not is_tuning: print("Starting MAPPO Ensemble training...")
        trainer_config = self.config['trainer']
        mappo_config = self.config['policy']['mappo_ensemble']
        buffer = MAPPOBuffer(
            obs_shape=(self.policy.policies[0].actor_mlp[0].in_features,),
            global_state_shape=(self.policy.policies[0].critic_mlp[0].in_features,),
        ...
        for epoch in range(trainer_config['epochs']):
            for t in range(trainer_config['steps_per_epoch']):
                ...
                raw_obs = obs
                featurized_obs = np.array([self.featurizer(o) for o in raw_obs])
                ...
                for i, agent_id in enumerate(learning_agent_ids):
                    action_mask_i = np.array(raw_obs[agent_id].get('legal_actions', np.ones(self.env.action_space.n)))
                ...
                llm_rewards = np.zeros(self.num_learning_agents)
                if self.llm_advisor and not is_tuning:
                    for i, agent_id in enumerate(learning_agent_ids):
                        ...
                        if strategy == 'on_steps':
                            if (self.total_steps - self.num_learning_agents + i) % self.llm_config.get('call_freq_steps', 10) == 0:
                                should_call, reason = True, "Step Freq"
                        elif strategy == 'on_uncertainty':
                            entropy = scipy.stats.entropy(action_probs_list[i])
                            threshold = self.llm_config.get('uncertainty_threshold', 1.5)
                            if entropy > threshold:
                                should_call, reason = True, f"Uncertainty ({entropy:.2f} > {threshold})"
                        if should_call:
                            prompt = format_prompt(raw_obs[agent_id], action_probs_list[i])
                            if prompt:
                                score, raw_response = self.llm_advisor.get_reward_score(prompt)
                                llm_rewards[i] = self.llm_config.get('lambda', 0.1) * score
                                current_trajectory.append({'prompt': prompt, 'response': raw_response})
                                print(f"\n>>> [LLM Advisor @ Agent {agent_id} @ Step {self.total_steps}] {reason}. Score: {score:.2f}. Reward: {llm_rewards[i]:.3f}\n", flush=True)
                ...
                env_rewards = np.array([rewards[i] for i in learning_agent_ids], dtype=np.float32)
                final_rewards = env_rewards + llm_rewards

                buffer.store(featurized_obs[learning_agent_ids], global_state, np.array(actions), final_rewards, 
                             value, np.array(log_probs), [done]*self.num_learning_agents, np.array(action_masks))

                obs = next_obs
                ep_returns += env_rewards
                ep_len += 1
                ...
                if done:
                    ...
                    is_win = info.get('result') == constants.Result.Win
            ...
            if not is_tuning:
                ...
                if (epoch + 1) % trainer_config.get('save_freq', 20) == 0:
                    save_name = trainer_config.get('save_name', 'mappo_ensemble_agent.pt')
        ...
        else: return sum(recent_wins) / len(recent_wins) if recent_wins else 0.0
    ...
    def run_evaluation_episodes(self, num_episodes, render=False):
        ...
        for i in range(num_episodes):
            ...
            if info.get('result') == constants.Result.Win and (not self.use_simple_opponents or self.training_agent_id in info.get('winners', [])):
                wins += 1
    ...
    def evaluate(self):
        print("Starting MAPPO Ensemble evaluation (LLM is disabled)...")
        ...
        eval_agent = MAPPOEnsembleAgent(config=self.config, _shared_policy=self.policy, _featurizer=self.featurizer)
