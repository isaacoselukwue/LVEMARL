...
from my_agents.happo_agent import HAPPOAgent

BASE_CONFIG = {
    'env': { 'scenario': 'PommeFFACompetition-v0' },
    'policy': {
        'happo': {
            'hidden_sizes': [256, 256], 'lr': 0.0003, 'gamma': 0.99,
            'clip_ratio': 0.2, 'value_coef': 1.0, 'entropy_coef': 0.01,
            'update_epochs': 10, 'batch_size': 256, 'tau': 0.005
        }
    },
    'trainer': {
        'steps_per_epoch': 2048, 'epochs': 30,
        'log_dir': 'logs/optuna_happo', 'save_dir': 'models/optuna_happo'
    },
    'evaluation': { 'num_episodes': 50 }
}

def objective(trial: optuna.Trial):
    """Optuna objective function for HAPPO."""
    trial_config = deepcopy(BASE_CONFIG)
    happo_config = trial_config['policy']['happo']
    ...
    agent = None
    try:
        agent = HAPPOAgent(config=trial_config, use_simple_opponents=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--study-name', type=str, default='happo_pommerman_study')
    parser.add_argument('--storage-db', type=str, default='sqlite:///trials/happo_study.db')
    ...
    if not any(t.state == optuna.trial.TrialState.COMPLETE for t in study.trials):
        ...
        best_params_config['policy']['happo'].update(best_trial.params)
        if 'layer_size' in best_trial.params:
            best_params_config['policy']['happo']['hidden_sizes'] = [best_trial.params['layer_size']] * 2
            del best_params_config['policy']['happo']['layer_size']
        yaml_path = os.path.join(os.path.dirname(args.storage_db.replace("sqlite:///", "")), "best_happo_params.yml")
