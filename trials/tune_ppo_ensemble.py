...
from my_agents.ppo_ensemble_agent import PPOEnsembleAgent
...
def objective(trial: optuna.Trial):
    """
    Optuna objective function for the PPO Ensemble.
    A "trial" consists of:
    1. Suggesting a set of hyperparameters.
    2. Running a short training session with them.
    3. Returning the final win rate as the metric to maximize.
    """
    trial_config = deepcopy(BASE_CONFIG)
    ppo_config = trial_config['policy']['ppo_ensemble']

    ppo_config['ensemble_size'] = trial.suggest_int('ensemble_size', 3, 7)
    ...
    agent = None
    try:
        agent = PPOEnsembleAgent(config=trial_config)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--study-name', type=str, default='ppo_ensemble_pommerman_study', help='Name for the Optuna study.')
    parser.add_argument('--storage-db', type=str, default='sqlite:///trials/ppo_ensemble_study.db', help='Optuna storage database URL.')
    ...
    if not completed_trials:
        ...
        ppo_params = best_params_config['policy']['ppo_ensemble']

        ppo_params.update(best_trial.params)

        if 'layer_size' in best_trial.params:
            ppo_params['hidden_sizes'] = [best_trial.params['layer_size']]
            del ppo_params['layer_size']

        yaml_path = os.path.join(os.path.dirname(args.storage_db.replace("sqlite:///", "")), "best_ppo_ensemble_params.yml")
